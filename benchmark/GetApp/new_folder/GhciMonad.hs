...
module GhciHaskeline.GhciMonad where
...
withVirtualPath m = do
  ghci_wd <- liftIO getCurrentDirectory                -- Store the cwd of GHCi
  st  <- getGHCiState
  liftIO $ setCurrentDirectory (virtual_path st)
  result <- m                                  -- Evaluate in the virtual wd..
  vwd <- liftIO getCurrentDirectory
  setGHCiState (st{ virtual_path = vwd})       -- Update the virtual path
  liftIO $ setCurrentDirectory ghci_wd                  -- ..and restore GHCi wd
  return result
