...
md5Finalize !ctx@(MD5Ctx (MD5Par _a _b _c _d) rem !totLen) =
...
performMD5Update !_ctx@(MD5Ctx !par@(MD5Par !a !b !c !d) _ !len) !bs =
...
applyMD5Rounds _par@(MD5Par a b c d) w =
        let -- Round 1
        ...
        {-# INLINE (!!) #-}
        getNthWord n _bs@(PS ptr off _len) =
